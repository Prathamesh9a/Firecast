  ...
  ({
    ...
    const toggleMute = useCallback(() => {
      if (videoRef.current) {
        ...
        console.log(`Video ${index} mute state: ${videoRef.current.muted}`);
      } else if (audioRef.current) {
        audioRef.current.muted = !audioRef.current.muted;
        setIsMuted(audioRef.current.muted);
        console.log(`Audio ${index} mute state: ${audioRef.current.muted}`);
      }
    }, [index]);
      ...
      if (videoRef.current) {
        ...
        videoRef.current.play()
          .then(() => {
            setRequiresInteraction(false);
            setError(null);
            console.log(`Video ${index} started after user interaction`);
          })
          .catch((err) => {
            console.error("Manual play failed:", err);
            setError({
        ...
        audioRef.current.play()
          .then(() => {
            setRequiresInteraction(false);
            setError(null);
            console.log(`Audio ${index} started after user interaction`);
          })
          .catch((err) => {
    ...
    const handleError = useCallback(
      (e) => {
        ...
        const errorDetails = {
          ...
          code: target.error?.code || "N/A",
          userAgent: navigator.userAgent,
          src: target.currentSrc,
        };

        if (target === videoRef.current && target.currentTime >= target.duration - 0.1) {
          console.warn("End of video reached. Reloading video source:", errorDetails);
          try {
        ...
        console.error(`${target.tagName} error:`, errorDetails);
        setIsLoading(false);
        setVideoReady(false);
        setError({
    ...
    useEffect(() => {
      ...
      const audioSrc = content.backgroundMusic ? getContentUrl(content.backgroundMusic) : null;
      if (content.backgroundMusic && audioRef.current && audioSrc) {
        setIsLoading(true);
        const audio = audioRef.current;
        audio.src = audioSrc; // Explicitly set the source
        console.log(`Audio ${index} setup: src=${audioSrc}, isMuted=${isMuted}, isPaused=${isPaused}, requiresInteraction=${requiresInteraction}`);

        const attemptToPlayAudio = () => {
          audio.play()
            .then(() => {
              setIsLoading(false);
              setIsMuted(false); // Ensure audio is unmuted after successful play
              console.log(`Audio ${index} playing: src=${audioSrc}`);
            })
            .catch((err) => {
              console.error(`Audio ${index} playback error:`, {
                message: err.message,
                name: err.name,
                src: audioSrc,
                userAgent: navigator.userAgent,
              ...
              if (err.name === "NotAllowedError") {
                setRequiresInteraction(true);
                setIsMuted(true);
                console.log(`Audio ${index}: Autoplay blocked, requires interaction`);
              } else {
                setError({
                  message: `Audio playback failed: ${err.message}`,
                  details: { code: err.code || "N/A", userAgent: navigator.userAgent, src: audioSrc },
                });
              }
            });
        };
        ...
        const handleCanPlay = () => {
          console.log(`Audio ${index} canplay: src=${audioSrc}`);
          setVideoReady(true);
          setError(null);
          if (!isPaused && !requiresInteraction) {
            attemptToPlayAudio();
            ...
            setIsLoading(false);
            console.log(`Audio ${index}: Playback skipped (isPaused=${isPaused}, requiresInteraction=${requiresInteraction})`);
          }
        };

        audio.addEventListener("canplay", handleCanPlay);
        audio.addEventListener("error", (e) => {
          console.error(`Audio ${index} error:`, {
        ...
        audio.addEventListener("playing", () => {
          console.log(`Audio ${index} is now playing: src=${audioSrc}`);
          setIsLoading(false);
        ...
        if (isPaused) {
          console.log(`Audio ${index}: Paused due to isPaused=true`);
          audio.pause();
        ...
        console.log(`Audio ${index}: No valid backgroundMusic or audioRef missing`, {
    ...
    }, [inView, content, isPaused, isMuted, requiresInteraction, handleError, index, getContentUrl]);
    ...
    useEffect(() => {
      ...
      const attemptToPlayVideo = () => {
        ...
        video.play()
          .then(() => {
            setIsLoading(false);
            console.log(`Video ${index} playing: ${video.src}`);
          })
          .catch((err) => {
            console.error(`Playback failed for video ${index}:`, err);
            setIsLoading(false);
            if (err.name === "NotAllowedError") {
              setRequiresInteraction(true);
              setIsMuted(true);
              setError({
                message: "Autoplay blocked: User interaction required for sound",
                details: {
                  code: "NotAllowedError",
                  userAgent: navigator.userAgent,
                },
      ...
      if (isPaused) {
        ...
        video.play().catch((err) => {
          console.error(`Initial playback failed for video ${index}:`, err);
          if (err.name === "NotAllowedError") {
            setRequiresInteraction(true);
            setIsMuted(true);
            setError({
        ...
        <div ref={ref} className="w-full h-full relative">
          {inView ? (
            <>
              ...
              <button
                onClick={toggleMute}
                className="absolute top-4 right-4 bg-gray-900 bg-opacity-80 text-white p-2 rounded-full shadow-lg transition-all duration-300 hover:scale-110"
              ...
              {audioSrc && (
                <>
                  ...
                  >
                    <source
                      src={audioSrc}
                      type={audioSrc.toLowerCase().endsWith(".mp3") ? "audio/mpeg" : "audio/wav"}
                    />
                  ...
                  {requiresInteraction && (
                    <button
                      onClick={handleManualPlay}
                      className="absolute bottom-4 left-1/2 transform -translate-x-1/2 bg-blue-600 hover:bg-blue-700 text-white py-2 px-4 rounded-md shadow-lg flex items-center gap-2"
                    >
                      <FaPlay size={20} />
                      Play Audio
                    </button>
    ...
    (content) => {
      (content || []).slice(0, 3).forEach((item) => {
        ...
        if (item?.backgroundMusic) {
          const audioUrl = getContentUrl(item.backgroundMusic);
          const link = document.createElement("link");
          link.rel = "preload";
          link.href = audioUrl;
          link.as = "audio";
          link.onerror = () => console.error(`Failed to preload audio ${audioUrl}`);
          document.head.appendChild(link);
        }
      });
    },
