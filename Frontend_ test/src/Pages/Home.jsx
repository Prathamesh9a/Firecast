...
// Sample audio files for the dropdown (update with actual files available in your project)
const audioFiles = [
  { name: "None", path: "" },
  { name: "Background Music 1", path: "/music/faith-in-love-30-sec-336746.mp3" },
  { name: "Background Music 2", path: "/music/magical-dramedy-orchestral-sneaky-spell-30-sec-375796.mp3" },
  { name: "Background Music 3", path: "/music/world-travel-30-seconds-368180.mp3" },
];
...
const Home1 = () => {
  ...
  const handleMusicSelect = (e) => {
    ...
    setSelectedMusic(selectedPath);
    if (selectedPath) {
      const url = selectedPath.startsWith('http') ? selectedPath : `${apiBaseUrl}${selectedPath}`;
      setMusicUrl(url);
      console.log(`Selected music: ${selectedPath}, URL: ${url}`);
      // Create a file-like object for FormData
      const fileName = selectedPath.split('/').pop();
      const file = new File([], fileName, { type: `audio/${fileName.split('.').pop()}` });
      dispatch(setMediaItems(groups.map(group => ({
        ...group,
        items: group.items.map(item => ({
          ...item,
          backgroundMusicFile: selectedPath ? file : null,
        })),
      }))));
    } else {
      setMusicUrl('');
    ...
    if (audioRef.current && musicUrl) {
      const playAudio = async () => {
        try {
          ...
          console.log("Music is playing:", musicUrl);
        } catch (err) {
  ...
  const handleCreateUrl = async () => {
    ...
    const links = groups.flatMap(group => group.items.map(item => ({
      ...
      backgroundMusic: selectedMusic, // Include selected music
    })));
    ...
    formData.append('userId', userId);
    if (selectedMusic) {
      const fileName = selectedMusic.split('/').pop();
      const file = new File([], fileName, { type: `audio/${fileName.split('.').pop()}` });
      formData.append('backgroundMusicFile', file);
      formData.append('backgroundMusicName', fileName);
    }
    ...
    links.forEach((linkItem, index) => {
      ...
      if (linkItem.backgroundMusic) {
        formData.append(`links[${index}][backgroundMusic]`, linkItem.backgroundMusic);
      }
    });
    ...
    try {
      ...
      // Update Redux with media items including background music
      dispatch(setMediaItems(links));
    } catch (error) {
      console.error('Error creating URLs:', error);
      setLoading(false);
      ...
      <div className="grid grid-cols-10 gap-2 bg-gray-100 p-2" style={{ overflow: 'hidden', height: '90vh' }}>
        ...
        <div className="container col-span-8 space-y-10" style={{ height: '100%', overflow: 'hidden' }}>
          <div className="bg-gray-100 p-2 justify-items-center" style={{ height: '100%', overflow: 'hidden' }}>
            ...
            <div className="flex flex-col w-[220px] mb-4">
              <label className="text-xs text-gray-600 font-medium mb-1">Background Music</label>
              <select
                value={selectedMusic}
                onChange={handleMusicSelect}
                className="border border-gray-300 rounded-md p-2 bg-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
